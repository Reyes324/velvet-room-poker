...
- [x] 68.5 all-in-for-less（有效后手小于当前下注额）定价二次修复，经两轮复核（2026-08-02）：详见 design.md「后续追加：all-in-for-less 定价修复」。第一版修复基于一个引擎造不出来的场景（`potSize < currentBet`）验证，被复核推翻并证实引入新回归（多人桌全下场景 AI 用接近 0% 胜率的牌也梭哈）；第二版（`evCall` 减去跟不起的差额里会被边池机制退还的部分 `uncalledExcess`，撤回 `potIfCalled` 里一个基于不可达场景加的错误裁剪）经独立复核验证：跟注/全下临界点精确落在真实底池赔率盈亏平衡点上，且能证明旧公式在同样输入下确实算错。已知局限：这版对"退款"的建模只精确处理一个对手，3 人及以上桌全下场景仍会算得偏乐观（复核给出具体例子：真实盈亏平衡点 33.3%，代码算出 0.66%），留作独立后续任务。服务端全量单测 228/228 通过
