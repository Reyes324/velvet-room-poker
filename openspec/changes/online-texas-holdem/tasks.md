...
- [ ] 28.1 `cardsSide` 方向从"推向所在列外侧"改为"推向中间通道"
- [ ] 28.2 英雄小头像座位 `y` 坐标重新实测确定，跟 `.hero-section` 不重叠
- [ ] 28.3 `.table-oval` 去掉 `border-radius:50%` 圆形装饰，绒面背景移到 `.table-canvas` 覆盖整个牌桌区域
- [ ] 28.4 Playwright 实测 2 人局（复现用户截图场景）+ 9 人满桌，确认暗牌不出画布、英雄信息区不重叠
