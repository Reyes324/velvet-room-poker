...
## 28. 真机实测：暗牌裁切 + 英雄信息区重叠 + 去掉椭圆装饰（2026-07-20）

- [x] 28.1 `cardsSide` 方向从"推向所在列外侧"改为"推向中间通道"
- [x] 28.2 英雄小头像座位 `y` 坐标重新实测确定（585→430），跟 `.hero-section` 不重叠，多视口（iPhone SE/14、地址栏可见短视口）验证留有安全余量
- [x] 28.3 `.table-oval` 去掉 `border-radius:50%` 圆形装饰，绒面背景移到 `.table-canvas` 覆盖整个牌桌区域
- [x] 28.4 Playwright 实测 2 人局（复现用户截图场景）+ 9 人满桌，确认暗牌不出画布、英雄信息区不重叠；两条断言写成永久 e2e 回归测试（`e2e/game.spec.js` "真机实测回归"）
- [x] 28.5（用户追加需求）下注气泡从"文字胶囊+尾巴"改成"筹码图标+金额"，去掉尾巴/`--tail-deg` 整套机制
- [x] 28.6（用户追加需求）英雄小头像卡片恢复显示筹码 footer，跟其他玩家的卡片样式保持一致（不再单独隐藏）；连带删掉 `.hero-section` 下面重复的 `.hero-chips`（两处都显示筹码是重复展示，用户反馈后去重）
- [x] 28.7（用户追加需求）暗牌尺寸缩小（`.c-xs` 24×34→17×24，密集桌 19×27→14×20）、贴座位间距收紧（`calc(100%+6px)`→`+3px`），不再遮挡自己的下注筹码
- [x] 28.8（用户追加需求）`BET_CHIP_OFFSET` 40→92（实测调出），下注筹码飞得比暗牌更远，两者不再抢同一块空间
- [x] 28.9（用户追加需求）公共牌区域 `.table-oval` 的 `bottom` 180px→200px，9 人满桌最下面一排座位的暗牌不再遮挡公共牌
- [x] 28.10（用户追加需求）新增 `rowPitchFor(rowsPerColumn)`：单栏 ≤2 排间距 130px，3 排 100px，4 排（7-9人）76px，人少时更松、人多才收紧
- [x] 28.11（用户追加需求）`.stack-chip-footer` 字重 700→400（不加粗）；加 `overflow:hidden`/`text-overflow:ellipsis` 防止大额筹码数字溢出卡片边框
- [x] 28.12 Playwright 实测 2 人局 + 9 人满桌两种场景验证暗牌不遮挡筹码/公共牌，三条新断言固化为永久 e2e 回归测试
