...
- [x] 64.4 ~~PVE 页面新增可展开战绩面板（玩家可见）：历史总局数、净输赢（复用 `debt`），按阶段/弃牌点的简单趋势展示~~ cancelled（2026-08-15，用户要求从"三个点"菜单去掉"战绩"入口——它是这个面板的唯一入口，移除后功能不可达，顺手删了 `PveStatsModal.jsx`/`showStats` 状态/`onOpenStats` prop/`.stats-*` CSS，见 design.md）
- [x] 64.5 单测覆盖：位置调整生效的分布差异、画像持久化的加载/写回、滚动窗口裁剪逻辑；~~真机 Playwright 验证战绩面板展示正确~~（面板已随 64.4 移除，这条验收项不再适用）
...
- [ ] **待做：语音对讲功能**（用户需求，2026-08-09）—— 方案已定稿，见 design.md「语音对讲功能」章节，完整设计见 `docs/superpowers/specs/2026-08-09-voice-intercom-design.md`
  - WebRTC 点对点直连（mesh）+ 复用 socket.io 信令，语音不经过服务器；按住说话（push-to-talk）
  - ~~借助开源库 `simple-peer` 处理连接建立/维护~~ **已推翻（2026-08-09 实测）**：该库最后发版 2023-01，停更三年半；且仅 `import` 一下就让**整站白屏**（`global is not defined`），而 `vite build` 照常通过——构建绿、真机死。改用浏览器原生 `RTCPeerConnection`，自检页里那 50 行原生协商代码已在真实浏览器跑通并有 e2e 守着
  - ~~明确不做：不接 TURN 中继兜底~~ **前提已被真机实测证伪，改为必须做**（见下方「双机实测第二轮」）；仍然不做：服务器端转发/录制、超过 8 人上限的扩展

- [ ] **待做：延迟优化——换一台更近的中继（真机实测第三轮，2026-08-09）**
  - **接手须知**：语音这条线的完整交接说明在 `docs/superpowers/specs/2026-08-09-voice-intercom-design.md` **开头的「交接说明」一节**——当前状态、三条已被实测推翻的判断、已落地文件清单、下一步顺序都在那里。接手前先读那一节，不要从这份 checklist 反推
  - **实测结果**：加中继后**跨网络通了**（用户："是可以的，只是延时有点长而已"）。可行性问题已解决，剩下的是优化问题，性质完全不同
  - **延迟来源是地理距离，不是方案本身**：免费公共 TURN 在境外，路径是「国内 → 境外 → 国内」。中继模式下 RTT 主要由到中继的距离决定，换一台更近的即可改善，**不需要改架构**
  - **先量再决定**：`/voice-pair` 加了往返延迟持续采样（`getStats` 的 `currentRoundTripTime`，1 秒一次 + 记峰值，卡顿由峰值造成而非均值）与实际用上的中继地址。"有点长"是感觉，200ms 和 600ms 对应的结论完全不同
  - 分档解读按**对讲**的容忍度写，不是打电话的标准——按住说话、说完松开本来就更耐延迟
  - **候选方案（待数字出来再选）**：①换一台更近的免费/低价中继（港/新），只改配置 ②自建 coturn 放港/新小型 VPS，约每月几十元，可控性最好 ③商用 TURN 按流量计费
  - **验收**：新增 e2e 1 条（往返延迟测得出真实数字且带人话解读——一个恒为空的仪表比没有仪表更糟，会让人以为延迟没问题）；voicePair 7 条全过；构建通过；eslint 与基线持平（39，零新增——中途多出的 1 条 `exhaustive-deps` 已修，没有带着新增警告走）

- [x] **跨网络连不上——接入 TURN 中继（真机实测第二轮，2026-08-09）**
  - **实测结果**：同一网络下**通**（真人听见真人说话，微信 iOS）；跨网络（一台 `NetType/4G`、一台 WiFi）**不通**，`ICE：failed`，对方声音峰值 0.000
  - **已经拿下的**：麦克风、WebRTC、我们自己写的信令与建连链路全都是好的。失败的只有 NAT 穿透这一环
  - **被推翻的是"极少数场景"这个前提**：跨网络不是边缘情况，而是这个产品的**主场景**——朋友各在各家、各用各的网络联机打牌，"同一个 WiFi"才是少数。国内运营商移动网络普遍 CGNAT 且多为对称型，STUN 学到的公网地址对对端无效，纯 P2P 打不通是必然而非偶然。**一个功能只在少数场景可用，等于不可用**，所以这不是范围膨胀，是原始假设被证伪后的必然修正
  - **成本结构变了**：TURN 是真的在中转音频、实打实消耗带宽，不可能像 STUN 那样白嫖。从"零运营成本"变成"可能要花钱"，是需要用户拍板的产品决策
  - **本轮只做低成本取证，不花钱**：先接免费公共 TURN（OpenRelay，80/443/443-TCP 三条，443/TCP 留给限制严格的网络）让用户复测跨网络。能通则证明"加中继确实能解决"，再谈免费额度够不够 / 付费 / 自建；不通则要单独查（免费 TURN 多在境外，境内可达性本身就是变量）。**拿到这一轮证据之前不做付费决定**
  - 自检页同步加 TURN 可达性探测：`probeStun` 泛化为 `probeIce`（STUN 看 `srflx`、TURN 看 `relay`，只差想要的候选类型，分两份写迟早一边修了另一边没修），TURN 用 `iceTransportPolicy:'relay'` 排除 host 候选干扰、超时单独放宽到 12 秒（要先认证再分配中继地址，用 STUN 的 6 秒会把可用中继误报成不可达）
  - 结论分档改为三档：中继可用 → 能做；只有直连可用 → **"同一个网络下能用，跨网络还没着落"**（真机第二轮正是这一档，原来这里会错误地报"可以做语音对讲"）；都不可用 → 网络这关整个没过
  - 双机页失败文案不再只说"没打通"（对用户零信息量），改为点出最可能的原因并给一个可执行的对照实验（两台连同一 WiFi 再试）
  - **又一次踩到同一个坑并当场改掉**：新写的提示文案里又用了 markdown 的 `**` 而那里是 JSX。这个项目已经为此加过 e2e 守卫，说明守卫有效但**写的时候仍会犯**——记在这里
  - **验收**：服务端全量 345 通过；e2e voiceCheck 4 + voicePair 6 全过（结论断言改为按实测结果三分支，不写死"必须全绿"，否则会变成一条跟着网络环境随机红的测试）；构建通过；eslint 与基线持平（39，零新增）
...
- [x] **双机连通性实测页 `/voice-pair`**（2026-08-09 真机实测后新增，已实现，**等用户真机实测**）
  ...
  - **服务端**：`voice:join` / `voice:signal` / `voice:leave` 三个事件，与游戏状态完全解耦（不碰 `rooms`、不碰 `myPlayerId`，成员关系交给 socket.io 自己的 room 机制）。只转发 SDP/ICE，**音频永不经过服务器**——这是整个方案能在 Render 免费档上白嫖的根本原因
  - **发起方由服务端一次性指定**（`shouldInitiate`：已在房间里的当接听方，后进来的当发起方）。两端各自猜会同时发 offer（glare），是这类实现最常见的卡死原因，而且症状是"偶发连不上"——漏了它，真机实测拿到的 no-go 结论就是假的
  - **信令只转发给同一语音房间内的成员**：否则任何人拿一个 socket id 就能往任意连接推消息，是白送的越权面，一行判断堵掉
  - 用 `disconnecting` 而非 `disconnect` 通知对端离开——`disconnect` 触发时 `socket.rooms` 已清空，那时已经不知道该往哪儿发
  - **客户端**：`/voice-pair`，URL 带 `?code=` 可直接命中（微信里发链接对方点开就用，手输 4 位数是实打实的流失点）；麦克风在**点击手势内**申请（微信硬性约束）；远端流 `muted:false` 真外放
  - **验收标准刻意定成"你听得见对方"而不是"音量条动了"**：iOS 上把远端流接到 AudioContext 量音量本身就不可靠，用一个可能撒谎的仪表当唯一判据，正是这两页一直在避免的事。音量条保留但只作辅助证据，结论区固定带上"最终以耳朵为准"，并有 e2e 守着这句话不被删掉
  - **顺手做的结构整理，不是新增**：`detectEnv`/`envLabel`/`micErrorText`/`attachSink`/`meterStream`/`SOUND_THRESHOLD` 与音量条组件从 `VoiceCheckPage` 抽到 `client/src/utils/voice.js` + `components/VoiceMeter.jsx` 两页共用。理由不是"文件太长"，而是这两页对同一件事必须给出**同一个**答案——门槛值各存一份就会出现"自检说有声音、双机说没有"这种自己打自己的结论，而这两页的全部价值就是结论可信
  - **写测试时抓到一个真 bug**：满员判断写成了 `members.length >= MAX - 1`，而 `members` 已排除自己，结果**第二个人就被拒**（"这个暗号已经有两个人在用了"），双机模式根本用不了。纯读代码不容易发现，测试第一版直接实测到
  - **测试里自己的坑，记下来**：`socket.io` 客户端 `close()` 之后 `socket.id` 会变成 `undefined`，断言前若不先把 id 存下来，就成了 `undefined === undefined` 的永真断言
  - **验收**：新增服务端 `voiceSignaling.test.js` **10 条**（发起方指定、peer-joined、信令原样转发、跨房间不转发、封顶 2 人、暗号格式、断开/主动离开通知、换暗号退旧房、不碰游戏状态）；新增 e2e `voicePair.spec.js` **6 条**——其中**最关键的一条用两个互相独立的浏览器上下文经真实服务端信令建连，并断言声音双向真的传到了**（这条绿 = 信令+建连+音频收发链路是通的，真机万一还连不上，问题范围可直接收窄到网络/微信内核，不必怀疑自己的实现）；服务端全量 **345** 通过；e2e 全量 **47 通过 / 4 失败**，那 4 条已在改动前的版本上复跑验证过**同样失败**（沙箱预装 Chromium 版本旧 + 机器慢，`page.goto` 超时），非本次引入；客户端构建通过；eslint 与基线持平（39，零新增）；390×844 真实截图确认连接态渲染正常（连接状态 `connected`、路径 `host ↔ host`、两条音量条均有读数、结论区正确）
...
- [x] **语音对讲接入实际牌桌**（2026-08-10，用户明确要求自主执行，不逐步确认）—— 方案见 design.md「接入实际牌桌」章节
  - 信令复用游戏房间（`voice-mesh:${roomCode}`），N 人 mesh，不再是 `/voice-pair` 那个 2 人暗号版本；服务端新增 `voice:mesh-join` / `voice:mesh-signal` / `voice:mesh-leave` / `voice:speaking`，与探路阶段的 `voice:join` 等事件互不干扰
  - "说话中"由 push-to-talk 按下/松开驱动广播，不靠远端音量分析
  - 麦克风只在第一次按下时申请；连接一开始是 `recvonly`，第一次按下时 `replaceTrack` 升级成 `sendrecv`，触发一次真实 renegotiation——用了标准的 perfect-negotiation（polite/impolite）模式避免双方同时重新协商时的冲突，之后每次按下/松开只是本地 `track.enabled` 切换，不再重新协商
  - 说话指示器是独立叠加层（`.speak-glow` + `.speak-badge`，不占用 `.avatar-card` 自身 `border`/`box-shadow`），绿色（`--state-safe`），跟倒计时环（金色）区分开，脉冲发光强度按验收标准调得比 `is-active` 呼吸更明显
  - **排查并修复的真实 bug（都是真机/真实浏览器跑出来的，不是读代码猜的）**：
    1. `useSocket.js` 的 `emit` 包装只转发 `(event, data)`，不支持 ack 回调——最初用它调 `voice:mesh-join` 时回调永远不触发，UI 卡死在"连接中"。改为该处直接用 `socket.emit`
    2. e2e 实测中 Playwright 的 `.click()` 在这套沙箱环境下对 `.voice-toggle-btn` 偶发会真的连续触发两次原生 `click` 事件（`isTrusted:true`，间隔约 270ms，用 `e.detail`/`e.timeStamp` 实测确认），一度误判为需要给按钮加防抖；后来发现这是自动化工具在这套环境下的已知抖动，不是真实用户操作会出现的模式，且防抖本身会误伤"开完立刻关"这种间隔更短但完全合法的真实交互（本机 WebRTC 建连极快，从开到关可能不到 200ms）。**改法在测试侧**：这几个交互改用 `dispatchEvent('click')`，产品代码保持原样的简单开关逻辑，不加不必要的防抖
    3. 测试脚本自己的设计错误：早期把"等待某人连接数涨到 N"和"点击开语音"耦合在同一个 helper 里，导致对已经开着语音的人重复点击、把它误关掉——拆成 `openVoice`（只点一次）和 `waitForPeerCount`（纯等待，不点击）两个函数后消失
  - **验收数据**：
    - 服务端新增单测 `voiceMesh.test.js` **10 条**全过（没进过牌桌拿不到 mesh 资格、三人桌两两拿到正确 playerId 列表且互相收到通知、信令带 fromPlayerId 且不转发给非同房间成员/未 join 的连接、说话广播只到同房间且不能冒充别人 playerId、断线与主动离开都通知对端且离开后不再转发信令、封顶 8 人与牌桌上限一致、不碰游戏状态）
    - 新增 e2e `voiceTable.spec.js` **4 条**全过，覆盖：三人桌两两建连（断言真实 `iceConnectionState`，不是只看信令）、按住说话在自己/对方/第三人屏幕上的正确表现、关闭语音后连接清理干净、两桌语音互相隔离；本地连续跑 3 轮确认稳定（0 flaky）
    - 服务端全量 **355** 通过；客户端构建通过；**e2e 全量 56 条全过**（含既有 game/lobby/serverReset/voiceCheck/voicePair/turnTimeout 各套件，零回归）；eslint 与基线持平（39，零新增）
    - 真实渲染截图确认了 5 个关键状态：未开语音、已开语音空闲、本人说话中（自己屏幕）、对方说话中（对方屏幕上看到我的座位发光）、说话叠加回合行动栏同时出现且互不遮挡；倒计时环与说话发光两者是独立叠加层这一条通过代码走查确认不冲突，未拍到两者同时点亮的巧合时机，不影响结论
  - **明确未做（本轮范围内已写清楚，不是遗漏）**：延迟数字仍未拿到（用户原话"先不管"）；8 人满桌的真实设备负担仍未实测（设计文档标注过的既有风险）；push-to-talk 是否妨碍出牌仍待真机验证（同上）；不做常开麦克风/持久静音切换；不做部分 mesh 连接失败时的用户可见错误提示文案

- [x] **语音 UX 修正：默认全员接听 + 音量驱动的说话波纹**（2026-08-10，真机联调后用户反馈，方案见 design.md「UX 修正」章节）
  - 顶部的 🎙️/🔇 手动开关整个移除，`useVoiceMesh` 挂载即自动 `enable()`，语音默认对所有人开启（只影响"听"，麦克风权限仍然只在第一次按住"说话"时才申请）；连带移除了 hook 里已经变成死代码的 `toggle`/`disable`
  - 顶部常驻的筹码数字删除（跟座位卡自己的 footer 重复，用户确认直接删，不移到别处）
  - 说话状态从"只有发光"升级成"发光（离散：在不在说话）+ 波纹（连续：多大声）"：新增 `.speak-ripple`，扩散幅度由 `--speak-intensity`（0..1）CSS 变量驱动，这个变量由 `PlayerSeat` 内部的 `requestAnimationFrame` 循环直接写在 DOM 节点 inline style 上，不经过 React state（音量是每帧都在变的数据，做成 state 会在整段说话期间反复重渲染整棵树）
  - 音量数据来自 Web Audio API 的 `AnalyserNode`：自己说话时接本地麦克风流，对方说话时接 WebRTC `ontrack` 拿到的远端流，取时域 RMS 乘系数标定到视觉友好的 0~1 区间；自己和所有对方共用同一张 `playerId -> volume` 的 Map，一个共享 rAF 循环统一采样
  - **顺带修的 e2e**：原来的三个测试依赖"逐个点击开语音、按顺序断言中间态连接数"（比如"Alice 先开时房间里没别人所以是 0"），这套顺序假设在语音变成自动接入后不再成立——三个人的 `enable()` 现在几乎同时触发，不再由测试控制先后。改成只断言最终收敛状态（每个人跟其余几人都建上连接），不再断言过程中的中间态。"关闭语音后连接清理干净"这条测试原来点一个现在已经不存在的按钮，改用关掉浏览器 context 模拟真实的"离开牌桌"（服务端 `disconnecting` 事件本来就独立于客户端主动 leave 处理这条清理路径，不依赖那个按钮）
  - **验收**：服务端改动为零（这轮完全在客户端，未碰信令协议）；客户端构建通过；eslint 与基线持平（39 problems / 30 errors，零新增）；e2e `voiceTable.spec.js`/`voiceCheck.spec.js`/`voicePair.spec.js` 三套共 15 条全过；服务端全量 355 条里有 2 条零星失败（`integration.test.js` 一条断连测试、`aiPersonaSimulation` 一条概率性弃牌率测试），单独重跑均通过，且改动前的原始代码上同样能复现其中至少一条——确认是既有的 timing/概率类 flaky，不是本次改动引入
  - **明确未做（本轮反馈范围之外）**：不做音量数据的服务端广播/持久化（纯本地实时视觉效果，没必要上网络）；不做"静音某个人"这类音频控制

- [x] **视觉打磨批次：桌布/牌背/牌面/菜单**（2026-08-11，用户拿到线上版本后一轮反馈，无需求文档提前设计，直接按反馈动手+确认后收尾）
  - **菜单图标**：左上角 `.menu-btn` 的 "≡"（hamburger）换成三个点横排（kebab/meatballs，用真 SVG 圆点画，不用 unicode 字符）——用户反馈国内用户对面包屑不熟悉；第一版做成竖排，用户纠正后改横排
  - **扑克牌**：数字下方的小花色符号（`.cs`）各档位放大约 25~30%，跟数字比例更协调；去掉牌桌上"翻牌前/翻牌圈/转牌圈/河牌圈"这行阶段文字（`Pot` 组件的 `street` prop 和对应 `PHASE_LABEL`/`.street-tag` 一并删除，不留死代码）——用户反馈是废话，桌面有几张公共牌本来就能看出阶段
  - **桌布**：没找到 PokerStars/GGPoker/WPT 公开的具体桌布配色（搜索确认这些客户端不公开设计规范），做了三个自定的低饱和度方向（柔化森林绿/深青绿/灰调鼠尾草绿）用本地 HTML 页面给用户对比选择。用户看完选定**深青绿**方向，同时反馈"想要毛茸茸质感"——在 `--felt-*` token 之外新增 `--felt-grain`（SVG `feTurbulence` 生成的噪点纹理，代码生成不引入图片资源），叠加在 `.table-felt`/`.table-oval-felt` 的径向渐变上（`background-blend-mode:overlay`）。同时把整条 ramp 的亮度跨度压小——原来径向渐变中心到边缘跳变大，被用户反馈成"中间泛白"，改用颗粒纹理表现质感而不是靠强对比制造立体感，用户明确要求"不要加其他太多效果"
  - **牌背**：原来是通用蓝色菱格纹，跟金/绒主题无关联。同样做了 7 个方向的本地对比页面（金线纹章/纯色徽记/酒红丝绒菱格/Art Deco 放射纹/双层金框/墨绿虚线边框等），用户选定**酒红丝绒 + 金色菱格绗缝纹**——`.c-back` 背景换成酒红渐变（`#6B1B28→#5A1420→#3A0D15`），菱格纹路和边框、中心徽记全部改成金色（呼应现有金色 UI 语言）
  - **首页菜单精简**：左上角原来有"问题反馈"/"刷新"/"最近更新"三个入口，用户反馈太碎，只留"问题反馈"。删除"刷新"和"最近更新"两个入口时，连带删除了因此变成死代码的 `ChangelogModal` 组件、`showChangelog` state、`.home-refresh-link`/`.home-changelog-link` CSS——`changelog.json` 数据文件本身保留（historical record，不是代码，不受这条清理影响）。用户澄清是首页菜单不是牌桌内菜单，牌桌内 `GameTable.jsx` 的"刷新页面"入口原样保留（同一个 iOS 主屏幕书签场景的兜底，不受这次反馈影响）
  - **方法论**：这轮桌布/牌背两项视觉方向性改动都是先做本地 HTML 对比稿（不直接改产品代码），列出多个方向 + 各自的取舍说明，用户看完再定，符合项目"UI 重设计先确认具体方案再动手"的既有规矩；小尺寸调整/明显是废话的文字/图标朝向这类低歧义反馈则直接动手改，不用先出对比稿
  - **验收**：构建通过；eslint 与基线持平（39/30，零新增）；e2e `lobby.spec.js` 9/9 全过，`game.spec.js` 23/25（2 条既有的旁观旁路 flaky 测试，在改动前的版本上同样失败，非本次引入）；真实渲染截图确认了菜单图标朝向、牌面花色尺寸、桌布颗粒质感+无泛白高光、酒红牌背、首页菜单精简这五项的实际效果
  - **后续来回（同一天晚些时候）**：桌布颜色又试了深青绿方向，用户觉得"不对"改回柔化森林绿；颗粒质感又觉得太重，调低强度、顺带发现并清理了一层没注意到的额外交叉纹理+独立噪点层+暖光高光层（真正生效的是 `velvet.css` 的 `.game-stage--table`，之前误以为生效的 `RoomPage.css` 那份 `.table-felt`/`.table-oval-felt` 其实是死代码）；最终用户决定**桌布颜色和牌背设计整个退回这轮改动之前的样子**（饱和度较高的原始绿 + 蓝色菱格纹牌背），全部由后台 agent 自主排查+实现+截图验证完成，不是逐步确认出来的

- [x] **暂停/继续功能**（2026-08-11，用户反馈，方案见 `docs/superpowers/specs/2026-08-11-pause-resume-design.md`，实施计划见 `docs/superpowers/plans/2026-08-11-pause-resume.md`）
...
- [x] **牌桌内"刷新页面"菜单项复核 + 顶部房间号展示**（2026-08-11，GitHub #14 真实用户反馈："左上角里面这个刷新有意义吗，没意义可以去掉，对了页面中间顶部是不是可以显示房间号，别人可以进来"）
  - **"刷新页面"复核结论：保留，不删除**。这条早前（同一天，见上面"首页菜单精简"条目）已经被明确讨论过一次——首页版本的"刷新"入口因为用户反馈太碎已删除，但牌桌内 `GameTable.jsx` 这一条是**刻意保留**的：iOS 加到主屏幕的书签是"独立 App"模式打开（`apple-mobile-web-app-capable`），没有 Safari 地址栏那套下拉刷新，切到后台再切回来经常只是原样唤醒上次冷启动加载的那个页面、不会重新请求——卡在这一局中途时，用户没有别的办法强制刷新页面。这个问题目前没有被其他机制解决（没有别的"检测到 stale session 自动重连/重渲染"逻辑）。用户这次反馈没看到过这段历史，只看到按钮本身觉得"没意义"——真正没讲清楚的是**为什么这条在菜单里**（跟账本/牌局记录/问题反馈这些游戏内功能混排，位置突兀），不是按钮本身没用。保留按钮和现有位置不动：菜单本身就是"杂项入口"的集合，刷新混在里面并不比其他任何一条更突兀，没有找到需要专门挪动或加说明文字的证据；真正要解决的是原始反馈的第二半——把房间号做成可见+可操作，让"别人可以进来"这件事不用非得走菜单。
  - **顶部房间号展示（新增）**：`GameTable.jsx` 的 `.top-bar` 新增房间号显示，位置在 `.menu-btn`（左）和 `.pause-btn`（右）之间——`roomCode` prop 早就传进来了但从未渲染。跟 `.timer-countdown`（计时游戏最后 5 分钟才出现的倒计时）复用同一个绝对居中定位槽位，两者互斥渲染（`countdownText ? 倒计时 : roomCode`）——倒计时本就是稀有场景（只在计时游戏最后 5 分钟出现），互斥即可，不需要额外挪位/收缩逻辑去"共存"，Playwright 实测确认二者互斥、不会同时出现在同一帧。
  - 点击房间号复制邀请链接（`${origin}/room/${roomCode}`），复用 `RoomPage.jsx`/`Lobby.jsx` 已有的 `copyInvite` 同款 `navigator.clipboard.writeText` 模式（未提炼公共函数——GameTable 和 RoomPage 是不同组件树，各自 3 行足够简单，提炼共享 hook 收益不明显），复制成功后原地把文案换成"已复制邀请链接 ✓" 2 秒（`codeCopied` state），沿用 `Lobby.jsx` 复制按钮同款的"原地换文案"确认模式，不是新发明一套。样式 `.top-room-code`（`velvet.css`）沿用 `.timer-countdown`/`.menu-btn` 同一套暗金圆角芯片语言。
  - **验收**：`cd client && npm run build` 通过；`npx eslint .` 38/29（≤ 基线 39/30，零新增，与拍一拍改动持平）；e2e 里没有测试引用"刷新页面"文案（`grep e2e/ 刷新页面` 无结果），保留该按钮无需改测试；新增 Playwright 临时脚本验证（未入库为正式 e2e，属一次性确认）：2 人房间开局后主机顶部正确显示 6 位房间号、跟 `.room-code`（大厅版本）文本一致、点击后原地变成"已复制邀请链接 ✓"、`navigator.clipboard.readText()` 确认剪贴板内容包含正确的邀请链接、且 `.timer-countdown` 不存在（非计时局），真机截图确认视觉效果（房间号芯片位于菜单按钮和暂停按钮之间，无重叠）。
  - **2026-08-15 补记：这条最终还是删了**。上面这条结论是"保留"，但这次用户（项目所有者本人，不是走 GitHub issue 反馈流程）直接要求从菜单去掉"刷新页面"——没有再提供新的替代兜底方案。之前保留的理由（iOS 独立 App 模式卡住无法刷新）本身没有被推翻，只是这次是所有者的直接决定，优先级高于早前的复核结论。

- [x] **座位填充顺序改成真顺时针（2026-08-12，用户反馈：庄家/小盲/大盲的座位关系看不出方向）**
  - **排查方式**：不接受手算——用 Playwright 起一个真实 6 人房间，读每个座位的 `.seat-name`/`.pos-badge`/`boundingBox()`，把庄家/小盲/大盲实际落在哪个屏幕坐标摆出来，才发现原来的 `twoColumnPositions()` 是"左一个右一个"交替填充（`opponents[0]→left row0, [1]→right row0, [2]→left row1…`），3 人以上时会左上→右上→左中→右中…来回横跳，不是顺时针绕一圈
  - **用户明确要求"位置不动，只改顺序"**——`twoColumnPositions()` 内部真正算坐标的部分（`columnYs()`/`spreadInZone()`/`TOP_ZONE`/`BOTTOM_ZONE`/pitch，这些都是之前拿真机截图一个个调出来的）一行没动，只改了循环怎么把 `opponents[i]` 分配到已经算好的那些槽位上：左列改成从"离 hero 最近"排到"最远"（原来 `columnYs()` 算出来的顺序是远到近，这里整个倒过来读），右列保持原来的近到远方向不变，两段拼起来正好是 hero→左侧从近到远→跨过桌顶→右侧从远到近→回到 hero，完整一圈顺时针
  - **验收**：同一套 Playwright 真机坐标验证脚本（3 人局 + 6 人局）复测，确认新顺序真的是顺时针（走位坐标记录在 commit message 里）；`cd client && npm run build` 通过；`npx eslint .` 与基线持平；`e2e/game.spec.js` 26/27 通过（唯一失败的"全下分出胜负后落败方归零"单独重跑通过，是依赖随机发牌结果的既有 flaky 测试，跟座位坐标无关，不是本次改动引入的回归）

- [x] **断线跨手自动离座（2026-08-12，用户反馈：一个朋友频繁掉线，但每一手仍照常被发牌参与）**
  - 排查结论：`connected` 状态从不影响发牌/回合倒计时（Bug C 修复后的既有设计），持续掉线的人会一直被发牌、一直靠 20 秒倒计时自动弃牌，白白送盲注，其他人每手都要陪等
  - 决策比早前"~~坐出~~"设计（design.md 对应节已标记取代）更简单：本手内断线不受影响；跨过一手边界仍未恢复 → 直接复用 `markLeft()`（账本保留）；重连走跟新加入完全一样的"旁观当前手→下一手重新入座"流程，不新增独立状态
...
- [x] **拍一拍表情：屏蔽🥚待换素材、新增☕️😈、选择器换行（2026-08-12）**
  - 砸蛋动画两版都不满意，找现成素材的浏览器自动化又卡住，先把 🥚 从选择器屏蔽（服务端白名单不删，只是客户端不显示），加 ☕️/😈
  - `.poke-picker` 从单行 flex 改成换行网格，踩坑记录：只设 max-width 不设 width 会导致"一行一个"而不是真正的多列换行，改成明确 width 才对
  - **验收**：真实 Playwright 确认无🥚、有☕️😈、9 个按钮、真换行、面板不超宽；构建/lint/服务端测试均无回归

- [x] **修订：表情选择器去掉纯拍一拍按钮、改真网格对齐（2026-08-12）**
  - 去掉 ✋（双击头像已有同等效果，选择器里是重复入口），剩 8 个表情
  - `.poke-picker` 从 flex-wrap 改成 `display:grid`（固定 32px 格子），修掉之前 flex 换行"看起来没对齐"的问题（根因是不同表情字形宽度不同，flex 换行只看内容宽度累加不看列）
  - **验收**：真实 Playwright 用渲染坐标验证像素级对齐（同行 y 相等、同列 x 相等）；双击仍能触发纯拍一拍；构建/lint/服务端测试无回归

- [x] **修订：砸蛋特效换用户提供的真实素材图 + 人机对战接上拍一拍（2026-08-13）**
  - 三版手画都不满意，用户直接发图，从会话上传目录取出、`sips` 压缩（1.4MB→140KB）存进 `client/src/assets/egg-splat.png`，🥚 重新放回选择器
  - 顺带修：PvePage.jsx 之前完全没接 `onPoke`，人机对战点头像没反应；本地 `useState` 实现同等的 `pokedSeat` 效果，不需要服务端广播
  - **验收**：真实 Playwright 确认图片正常加载（非 404）、人机对战场景下拍一拍能正确触发；构建（素材图正常打包）/lint/服务端测试/既有 e2e 均无回归

- [x] **修订：顶排座位气泡不探出视口 + 蛋从桌子中心飞过来（2026-08-13）**
...
- [x] **打字聊天真实收发（2026-08-15，方案见 design.md「语音/打字吸附组件」章节的后续实现）**
  - 服务端 `RoomManager.chat(fromId, text)` 照抄 `poke()` 的节流模式（`chatCooldowns` Map，2 秒冷却，冷却期内静默丢弃不报错），区别是按发送者单独计（不是拍一拍那种按有序对计），因为聊天是广播不针对某人；空消息报错，超长（>40 字）静默截断不报错。`server/index.js` 新增 `chat:message` handler，广播给全房间，不持久化
  - 客户端复用拍一拍的"单气泡 + key + 定时自清"状态机制（`RoomPage.jsx` 的 `chatBubble`），不是另起一套；气泡样式新增 `.poke-bubble--chat`（蓝色系，跟吸附组件"打字"分区同色相），停留时长 4s（比拍一拍的 1.6s 长——一整句话比一个表情需要更久阅读时间）
  - `VoiceChatDock.jsx` 的"打字"分区点击后弹出一个独立于胶囊本身的输入条（紧挨着胶囊同一条边浮出，不是塞进胶囊内部撑变形），Enter/点发送提交，Escape/失焦取消
  - **踩坑记录**：气泡文本渲染出来后每个字各占一行——根因是气泡的绝对定位容器是座位卡本身（~50~70px 宽），`width:fit-content` 会把容器可用宽度也算进候选值，直接把气泡压窄到跟座位一样窄，叠加 `word-break:break-word` 就成了逐字换行；改成 `width:max-content`（不看容器宽度，只按内容自然宽度算，`max-width` 兜底超长文本）解决。真实渲染截图发现的，不是靠读代码猜的
  - **验收**：服务端新增 6 条 vitest（成功/空消息/超长截断/冷却/冷却按发送者隔离/冷却过期后可再发），全量 393 条里 392 通过、1 条失败是已知的 `integration.test.js` 时序 flake（重跑必过，跟这次改动无关，此前排查过）；客户端构建/lint 持平基线；真实 Playwright 双人房间验证发送→双方都收到→4s 后气泡消失，且气泡单行正确渲染（截图确认修复了上面那个换行 bug）

- [x] **聊天气泡说话感 + 一批真机反馈修复（2026-08-15，方案见 design.md 同名章节）**
  - 加了漫画对话框式尾巴（按 `bubbleSide` 自动选朝下/左/右，指向真正的说话人，不是写死方向）+ 嵌入版式的装饰引号（不再是外挂圆徽）+ 专属"从尾巴生长"入场动画
  - 修了尾巴渲染成菱形（换成标准 border-triangle 手法）、尖角加圆润、气泡本体和尾巴颜色不统一、文字因为给引号让位的 padding 而没真居中
  - **真机测试发现的真问题**：40 字长消息在旧的 150px 宽度下换 4 行会盖住公共牌区域，`max-width` 提到 220px 压缩到约 2 行解决；拍一拍/聊天气泡原来固定 z-index 导致不管谁先发生某一类型永远压另一类型，改成直接用各自的 `Date.now()` key 当 z-index，按真实发生时间排序
  - 手机键盘加 `enterKeyHint="send"`，回车键显示"发送"而不是默认的换行图标
  - **已知未处理**：2 人局个别对手座位离顶部栏太近，长气泡边缘会贴到顶部栏，纯视觉不挡点击，根因在共用的座位分区系统，未来单独排期
  - **验收**：服务端全量 393 条通过（除已知 flake）；客户端构建/lint 持平基线；真实 Playwright 分别验证本人视角/对手视角尾巴方向、长文本 2 行不撞公共牌

- [x] **触摸尺寸恢复 + 键盘弹出两处修复（2026-08-15，方案见 design.md 同名章节）**
  - 吸附组件收起/展开尺寸从压缩版恢复到接近原说话按钮（58×72）量级：收起 46px、展开 50×150px
  - 修了键盘弹出把整个牌桌顶起来的 bug——根因是 `viewportHeight.js` 监听 `visualViewport` resize 来修 iOS 首屏留白，键盘弹出也会触发同一事件；改成用 `window.innerHeight`（键盘不影响）跟 `visualViewport.height` 的差值判断是不是键盘导致，是则跳过这次高度更新
  - 聊天气泡入场动画拆成"外层只定位、内层只动画"两层结构，不再让定位用的 transform 和动画用的 scale/translate 共用一个元素——真机反馈本人视角动画原点看着不对、对手视角看不出动画，怀疑是两套 transform 相关样式组合在实机上表现不稳定，没深挖具体机制，直接换更不会出这类问题的结构；顺带把按尾巴方向分三套 transform-origin 的做法简化成统一从气泡中心缩放
  - `enterKeyHint="send"` 手机键盘发送键改成"发送"文案
  - **验收**：服务端全量 393 条通过（除已知 flake）；客户端构建/lint 持平基线；真实 Playwright 确认气泡拆层后位置/尾巴/换行都没有回归

- [x] **打字气泡三处收尾（2026-08-15）**
  - 磨砂+半透明的争议来回改了好几轮之后彻底放弃——`background:#FBF6EC` 实色 + `border:1.5px solid var(--gold-400)`，"盖没盖住"从此是纯 CSS 层面能保证的事实，不用再靠沙盒测不出的 backdrop-filter 打赌
  - 收起/展开触摸尺寸又加大一档（46→58px，展开 50→58×150→180px），对齐到原 `.ptt-btn` 自身的宽度
...
- [x] **部署：定时保活，防止 Render 免费档休眠（2026-08-16，方案见 design.md 同名章节，含一次方案修正）**
  - 最初用 `.github/workflows/keep-alive.yml`（`cron '*/10 * * * *'`），实测不可靠——`gh run list` 看到自动触发间隔实际有 32-34 分钟的空窗，跟配置的 10 分钟对不上，GitHub 官方说明这是对不活跃仓库的已知限制
  - 排查过程中一度怀疑 TLS 握手慢（`curl` 拆解看到 5.18 秒），用 `openssl s_client` 连续测 3 次发现第一次慢、后两次瞬间完成，确认是本地测试环境缓存假象，撤回怀疑
...
- [x] **Bug 修复：微信内置浏览器悬浮前进/后退条盖住操作区（2026-08-16，方案见 design.md「URL 路由」章节的「修正」部分）**
  ...
  - **验收**：服务端全量 395/395 通过；客户端构建通过；用户真机微信实测确认悬浮条不再出现，操作区完整可点

- [x] **Bug 修复：微信内置浏览器底部露白边（2026-08-16，方案见 design.md「URL 路由」章节同一条「修正」的追加部分）**
  - **背景**：上面那条悬浮条修好之后，用户截图发现底部还有一道细白边——这是被悬浮条挡住、一直没暴露出来的另一个问题
  - **根因**：`viewportHeight.js` 里"加载时悄悄滚一下逼 WebKit 重算真实可视高度"这个修 iOS 首屏留白 bug 的技巧，之前只在 `navigator.standalone`（加到主屏幕独立打开）下才触发——但这是通用 WebKit 引擎 bug，不是独立模式专属，微信内置浏览器这种普通嵌入式场景一样会算错高度，只是从来没走到这段修复逻辑
  - **修法**：去掉 `if (window.navigator.standalone)` 限制，所有场景加载时都跑一次这个"滚一下重算"的技巧（仍然只在加载时跑一次，不是持续监听，不会跟已经踩过坑的键盘 resize 问题混在一起）
  - **验收**：服务端全量 395/395 通过；客户端构建通过。**真机微信验证还没做**——需要用户在原设备上实测确认底部白边消失
...
- [x] **房间闲置失效时间从 12 小时缩到 1 小时（2026-08-17，用户反馈，方案见 design.md「同设备退出后重新加入自动恢复身份 + 房间闲置...」章节的修订部分）**
