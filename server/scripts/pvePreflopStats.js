...
//
// 判读标准（本轮验收）：
//   - PFR 落在 15-25% 附近 = 像真人——但这是**满桌（full-ring，约 8-9
//     人）**的参考值，人数越少正确的开局范围本来就越宽。本脚本里人类座位
//     每手都直接弃牌，所以 seatCount=4 实际上是 3 人桌在互相对抗，3 人桌
//     下 PFR 40%+ 才是正确打法，不是缺陷。**要判断 PFR 是否落在真人区间，
//     必须用 seatCount=8 跑**，4 人桌等小桌的数字仅供参考，不能拿来对照
//     15-25% 下结论（2026-08-03 曾经因此错判过一次，见 task-5 报告）。
//     100% = 什么牌都加注（2026-08-03 之前的状态，任何桌型下都不正常）。
//     接近 0% = 矫枉过正成了岩石，同样任何桌型下都不正常。
...
function main() {
  ...
  console.log(
    ...
    + '   （真人参考区间 15-25% 是 full-ring/满桌数字；桌子越小正确范围本来就越宽——'
    + (seatCount >= 8 ? '本次 seatCount>=8，可以直接对照 15-25%'
      : `本次 seatCount=${seatCount}（人类座位每手弃牌，等价于更少人桌），偏高不代表有问题，要判定是否落在真人区间请用 seatCount=8 重跑`)
    + '；100% = 什么牌都加注；接近 0 = 矫枉过正）',
  );
