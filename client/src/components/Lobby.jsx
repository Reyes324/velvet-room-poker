...
export default function Lobby({ roomState, playerId, onCopy, onKick, onStart, onRestart, onRebuy, onExit, onOpenLedger, onOpenHandHistory, onOpenChatHistory, copied, maxSeats = 9 }) {
  ...
  return (
    <div className="game-stage">
      <div className="top-bar">
        ...
        <div className="bankroll"><ChipIcon />{(me?.chips ?? 0).toLocaleString()}</div>
      </div>
      {showMenu && (
      ...
      <div className="lobby">
        <div className="lobby-head">
          <div>
            ...
            <div className="lobby-blind">BLIND <ChipIcon />10 / <ChipIcon />20</div>
          </div>
            ...
            <div key={p.id} className="pl-row">
              ...
              <div className="pr-info">
                ...
                <div className="pr-chips">
                  {p.chips === 0 ? <span style={{ color: '#E08A4A' }}><ChipIcon />0 · 筹码不足</span> : (<><ChipIcon />{p.chips.toLocaleString()}</>)}
                </div>
              </div>
              {roomState.hostId === p.id && <span className="pr-badge">房主</span>}
              {p.debt > 0 && <span className="pr-badge debt-badge">借<ChipIcon />{p.debt.toLocaleString()}</span>}
