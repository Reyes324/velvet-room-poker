...
## 后续追加：all-in-for-less（有效后手小于当前下注额）定价修复（全面审查 + 两轮复核，2026-08-02）

**问题**：全分支最终审查（真实跑对局，不是单看公式）发现 AI 会把胜率 80% 的牌，在"筹码不够、只能全下一部分"的场景下弃掉——纯送筹码，直接违背"AI 应该想赢"这个前提。第一版修复被下一轮更严格的复核推翻：用来复现问题的测试场景本身（`potSize < currentBet`）在引擎里造不出来（`GameEngine._placeBet` 每次下注都立刻计入 `this.pot`，任何可达局面下 `potSize >= currentBet` 恒成立），基于这个不存在的场景做的修复反而引入了新回归（多人桌全下场景里，AI 会用接近 0% 胜率的烂牌也去全下）。第二版修复（`evCall` 减去"跟注差额里自己付不起、会被边池机制退还、永远赢不到的那部分"`uncalledExcess`；`potIfCalled` 撤回一个基于不可达场景加的裁剪）经过独立复核验证：跟注/全下的临界点精确落在真实底池赔率的盈亏平衡点上（手算 + 脚本双重核对），且能证明"用旧公式跑同样输入确实会算错"，不是巧合躲开。

**已知局限（复核发现，诚实记录，已于当天修复，见下一节）**：这版修复对"退款"的建模只精确处理了**一个对手**的情况（`uncalledExcess` 只减一次）。真实多人桌全下时，`GameEngine._buildSidePots` 会把超额部分退给**每一个**下注比我方全下额度高的对手，人数是 n 时应该退 n 次，不是 1 次。复核构造出一个可达的 3 人桌短筹码场景：代码算出来的盈亏平衡点是 0.66%，真实应该是 33.3%——这种场景下 AI 还是会用接近 0% 胜率的牌全下，只是程度比"完全不建模退款"轻。

**验证**：`pveStrategy.test.js` 里 all-in-for-less 相关的 4 个测试全部改成用可达参数（`potSize >= currentBet`），并且新增一个专门针对这次回归的测试，能证明用旧公式跑会得到错误答案。服务端全量单测 228/228 通过。

## 后续追加：all-in-for-less 多人桌退款按对手数缩放（2026-08-02）

**修复**：`evCall` 的 `uncalledExcess` 改成乘以 `liveOpponentCount`（还没弃牌的对手都各自退一份，不是只退一份）；`evRaise` 的 `potIfCalled` 里"对手的追加投入/退款"这一项，**只在 all-in-for-less 场景（`raiseCandidate < currentBet`）下**同样乘以 `liveOpponentCount`——正常加注场景（`raiseCandidate >= currentBet`）维持这次全面审查里已经独立验证过的口径不变，不连带修改（那个口径本身要不要也按人数缩放"若被跟注的底池"是一个独立问题，参见下面的"仍未做"记录）。两处都用 `Math.max(0, …)` 兜底"底池不能是负数"这个恒成立的物理下限，不是针对某个不可达局面的特判。

**验证**：手算 + 脚本双重核对——两人各下 1500、潜池 3000、自己剩 10 筹码的 3 人桌场景，修复后盈亏平衡点精确落在 33.3%（2%/30% 胜率弃牌，35% 胜率全下），跟复核给出的"真实应该是 33.3%"完全吻合；单人（`liveOpponentCount` 默认 1）场景数值不变，向后兼容。`pveStrategy.test.js` 新增两条测试：一条验证多人桌 all-in-for-less 的退款按人数缩放且临界点精确，一条验证正常加注场景的 `potIfCalled` 没有被连带改动（复用"弃牌权益随对手数量指数下降"那组测试的参数，确认加注 EV 没有因为这次改动意外翻正）。服务端全量单测 230/230 通过（`integration.test.js` 的 `room:end-game` 是已经多次记录过的全量套件下偶发 flake，单独跑 27/27 必过，跟这次改动无关）。

**仍未做（明确记录，不是漏了）**：正常加注（非 all-in-for-less）场景的"若被跟注的底池"要不要也按 `liveOpponentCount` 缩放——如果假设多人桌里所有还没弃牌的对手都会跟注同一个加注（这其实是 `estimateFoldEquity` 的聚合弃牌权益模型本来就在做的"要么全弃要么全跟"简化的自然延伸），那正常加注的"若被跟注"的底池确实也该更大。这次没有一起改，是因为这个口径已经在更早一轮独立验证过，没有具体证据证明它错，贸然改动风险大于收益——留作独立问题，需要单独找到一个能验证对错的具体场景再动。
