...
**已知的取舍/局限（诚实记录，不是没想到）**：
- ~~范围建模只在翻前生效，不随翻后牌面动态调整~~ ——已在下面"后续追加：翻牌后范围继续收窄"一节里做了一版（按"这手牌里连续几条街都面对加注"收窄，不是真正按牌面强弱重算范围，见该节的取舍说明）。
...
## 后续追加：感知噪声，解决"打法一刀切"（用户需求，2026-08-02）

**背景**：整体审查里 Opus 复核指出 AI 打法"接近确定性阈值函数"——同一个胜率区间几乎总做同一个动作（观测样本里单个区间纯度经常 ≥95%），长时间打下去容易被摸出规律。用户明确要求："加一点随机色彩？加点情绪，让这个比较像人类"。

**方案**：不是针对某个具体场景（比如"强牌该不该慢打"）单独加一条特判规则，而是给 EV 判断本身加一层"感知噪声"——`pickAction()` 选动作前，给算出来的 `evFold`/`evCall`/`evRaise` 各自加一点跟底池成比例的高斯噪声（Box-Muller 变换，标准差 = 底池 × `EV_NOISE_FRACTION`，建议值 0.15），再比大小，而不是直接比精确值。效果：EV 差距悬殊的决策几乎不受影响（一点噪声翻不动大的差距，比如 95% 胜率的坚果牌不管怎么加噪声都还是加注）；EV 接近的边界决策会自然变得不那么可预测——包括"强牌几乎从不过牌/慢打"这类之前纯粹靠公式精确比较导致的"一刀切"，是同一个机制的自然结果，不需要再专门为"慢打"写一条独立规则。

**接口**：`pickAction()` 新增可选 `noiseFraction` 参数，默认 `0`（`gaussianNoise` 在 `stdDev<=0` 时直接返回 0、不消耗 `random()`）——这几十个手算验证过精确边界值的既有单元测试全部不用改，行为完全不变。`PveSession.aiAction()` 给真实对局的决策显式传入 `EV_NOISE_FRACTION`（从 `pveStrategy` 导出）来开启，不作为 `pickAction` 的默认行为。诈唬层判断"要不要诈唬"用的还是未加噪声的客观胜率 `eq`，噪声只影响"EV 接近时选哪边"这一步，不影响"是否要诈唬"这个独立判断。

**验证**：脚本核对——同一个近似打平的局面（`evCall≈60, evRaise≈59.88`），关掉噪声时不管 random 序列是什么都稳定选 call；开启噪声后不同 random 序列会得到不同结果（call/raise/fold 都出现过）；95% 胜率的坚果牌开启噪声后所有测试序列依然稳定选 raise。新增 3 条单测（默认关闭时的向后兼容性、开启后边界决策的不确定性、开启后决定性决策的稳定性）+ 1 条 `PveSession` 侧确认 `noiseFraction` 真的被传给 `pickAction` 且不是默认的 0。服务端全量单测 234/234 通过，客户端构建通过（本次只涉及服务端）。

**明确不做（这次没有一起做的延伸想法，供以后参考）**：讨论过"心情值"（连赢加更敢冲、连输后更谨慎）这类跨手牌的状态化情绪机制，比这次的"每次决策独立加一点噪声"复杂得多，需要新增按 AI 坐位的跨手牌状态追踪。这次范围只做无状态的感知噪声，情绪状态化留作独立话题。

## 后续追加：翻牌后范围继续收窄（backlog 遗留项，2026-08-02）

**问题**：`opponentRangePct` 之前只由跨局累计的 `opponentAggressionRate` 算一次，同一手牌里不管对手连续几条街都在下注/加注（"barrel"），范围宽度都不变——对手翻前加注、翻牌下注、转牌又下注，这种持续施压通常意味着牌更强，理应比"只在翻前判断过一次"更能确认，但之前完全没建模这个信号。

**方案**：`PveSession` 新增 `handAggressionStreets`——按 AI 坐位记录"这手牌里、轮到它决策时已经在哪些街真的面对过加注"的 `Set`，每手开局（`_dealNewHand`）清空。`aiAction()` 面对加注时，把当前街加进这个坐位的 Set，用 `streets.size` 算一个指数收窄系数：`0.75 ** max(0, size-1)`。第一次面对加注（本手牌只在这一条街面对过）系数是 1，不额外收窄，跟之前行为一致；之后每多一条街还在面对加注，再乘 0.75，最终结果仍然 clamp 到 `[0.10, 0.90]`。同一条街内重复面对加注（比如翻前的 3bet/4bet war）不重复收窄——`Set.add` 同一个街名是幂等的，只在真正推进到新的一条街时才会让 `size` 变大。

**取舍**：这不是"按真实牌面强弱重新算范围"（比如翻出 2-2-2 之后该往哪个方向调整对手的范围），只是"持续施压 → 范围更窄"这一个更简单、更容易验证对错的信号；真正的、按牌面纹理重算范围的系统复杂得多（需要判断这个牌面对哪些起手牌类别有利/不利），仍然超出这轮范围，留作独立话题。

**验证**：`PveSession.test.js` 新增一条测试——把 `opponentAggressionRate` 钉在不贴边界的 0.5，验证翻前面对加注时 `opponentRangePct` 恰好是 0.5（streak=1，不收窄），翻牌上再次面对加注时精确是 `0.5 × 0.75 = 0.375`（streak=2）。服务端全量单测 235/235 通过（含这条新测试），这次全量跑没有出现已知的 `integration.test.js` flake。
