...
**已知局限（复核发现，诚实记录）**：这版修复对"退款"的建模只精确处理了**一个对手**的情况（`uncalledExcess` 只减一次）。真实多人桌全下时，`GameEngine._buildSidePots` 会把超额部分退给**每一个**下注比我方全下额度高的对手，人数是 n 时应该退 n 次，不是 1 次。复核构造出一个可达的 3 人桌短筹码场景：代码算出来的盈亏平衡点是 0.66%，真实应该是 33.3%——这种场景下 AI 还是会用接近 0% 胜率的牌全下，只是程度比"完全不建模退款"轻。这是比改之前更准、但仍不完整的状态，留作独立的后续任务，不在本轮继续追。
