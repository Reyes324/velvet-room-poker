...
**背景：** 本轮的主要验收标准是**双向**的：PFR 要落进真人区间（约 15-25%），**同时**不能矫枉过正把 AI 压成什么都不敢做的岩石。

> 【实施期修正，2026-08-03】**这个 15-25% 是满桌（`seatCount=8`）的参考值，评估必须用满桌。** 脚本让人类座位每手弃牌，`seatCount=4` 实际是 3 人短桌，短桌正确开池范围本来就有 40%+。实施时先只测 4 人桌得 PFR ~40-49%，据此误判"没达标"并触发下面的第三条分支；改测 8 人桌后两次独立运行得 PFR 16.3% / 17.3%、VPIP 22.1% / 23.8%、翻后主动率 38.2% / 40.9%，双向判据均达标，对应的是下面的**第一条**分支（0.6 保持不动）。下面三条分支判据本身仍然有效，只是必须在满桌数据上套用。另外 `POT_CONTROL_DISCOUNT`（0.6）当初是"缺失风险模型时的代理"，组件 A 是同一个病根的正解，两者叠加有过度保守的风险——**必须用数据决定它的去留，不许拍脑袋**。
